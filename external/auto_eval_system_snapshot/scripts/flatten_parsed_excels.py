...
SHEET_NAMES_CN = {
    "D1_Outpatient_Loop": "门诊循环",
    "D1_Outpatient_Decision": "门诊决策",
    "D2_Admission_Loop": "入院循环", 
    "D2_Admission_Decision": "入院决策",
    "D3_Surgery_Decision": "手术决策",
    "D4_Rehab_Plan": "康复计划"
}
...
def determine_status(row, sheet_name, sheet_idx, all_original_dfs):
    case_id = row.get('病例ID', None)
    
    # 获取当前行的医生JSON列（排除展开后的列）
    doctor_json_cols = [c for c in row.index if '医生' in c and 'JSON' in c and not c.endswith('_JSON_') and '医生_原始JSON' not in c.replace('医生_原始JSON', '')]
    
    # 检查当前行是否有非空数据
    current_has_data = False
    for col in doctor_json_cols:
        val = row[col]
        if pd.notna(val) and str(val).strip() != "" and str(val).strip().lower() != "nan":
...
def process_excel(file_path):
    try:
        ...
        for sheet in xls.sheet_names:
            all_original_dfs[sheet] = pd.read_excel(file_path, sheet_name=sheet)
        ...
        for sheet_idx, sheet_name in enumerate(SHEET_ORDER):
            ...
            all_json_cols = [c for c in df.columns if "原始JSON" in c]
            doctor_json_cols = [c for c in all_json_cols if "医生" in c]
            
            if not doctor_json_cols:
                continue
            ...
            cols_to_keep.extend(doctor_json_cols)
            
            subset_df = df[cols_to_keep].copy()
            
            final_concat_list = [subset_df]
            
            for j_col in doctor_json_cols:
            ...
            for idx, row in final_df.iterrows():
                status_text, fill_color = determine_status(row, sheet_name, sheet_idx, all_original_dfs)
        ...
        output_path = file_path.replace("_CN.xlsx", "_CN_Parsed.xlsx")
